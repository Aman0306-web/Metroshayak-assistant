...
# We now use dmrc_complete as the single source of truth
try:
    import dmrc_complete
except ImportError:
    print("⚠️ dmrc_complete.py not found! Please ensure it exists.")
    dmrc_complete = None
...
# Import rapidfuzz for NLP Intent Matching
try:
    from rapidfuzz import process, fuzz
except ImportError:
    process = None

# Define placeholder for STATION_LOADER to prevent NameError in endpoints
STATION_LOADER = None
...
SECRET_KEY = "dmrc_super_secret_key_2026"  # Change this in production!
ALGORITHM = "HS256"
...
fake_users_db = {
    "traveler": {
        "username": "traveler",
        "full_name": "Metro Traveler",
        "hashed_password": pwd_context.hash("metro123"),
        "disabled": False,
    }
}
...
async def startup_event():
    ...
    # Sync METRO_DATA from dmrc_complete (The Single Source of Truth)
    if dmrc_complete:
        print("🔄 Syncing METRO_DATA from dmrc_complete...")
        updates = 0
        for line_name, stations in dmrc_complete.METRO_NETWORKS.items():
            ...
            if key in METRO_DATA:
                ...
                # Add new line found in dmrc_complete
                METRO_DATA[key] = {
                    "name": line_name,
        ...
        print(f"✅ Synced {updates} lines from dmrc_complete to internal METRO_DATA")
...
class ChatbotEngine:
    ...
    def match_intent(self, message_lower: str) -> str:
        ...
        # 1. Check Hindi/Hinglish keywords first
        for word, intent in self.hindi_keywords.items():
            if word in message_lower:
                return intent

        # 2. Advanced NLP Fuzzy Matching (if rapidfuzz is available)
        if process:
            ...
            # Find best match
            match = process.extractOne(message_lower, all_examples, scorer=fuzz.WRatio)
...
def chat(msg: ChatMessage):
    ...
    # Try dmrc_complete chatbot logic first (it handles routes/fares well)
    if dmrc_complete:
        dmrc_response = dmrc_complete.process_chatbot_query(msg.message)
        if "🤖" not in dmrc_response: # If it's not the default fallback
            return {"response": dmrc_response, "language": msg.language}
...
def get_live_status():
    """Simulate live DMRC updates"""
    statuses = [
        {"line": "Yellow Line", "status": "Normal", "delay": 0, "message": "Service is normal."},
        {"line": "Blue Line", "status": "Delay", "delay": 10, "message": "Slow movement due to track maintenance."},
        {"line": "Red Line", "status": "Normal", "delay": 0, "message": "Service is normal."},
        {"line": "Airport Express", "status": "Normal", "delay": 0, "message": "Service is normal."}
    ]
    
    # Randomly pick a "Live" alert for demo
    alert = random.choice([
        None,
        "⚠️ Blue Line: 10 min delay between Rajiv Chowk and Noida.",
        "ℹ️ Gate No. 2 at Central Secretariat is closed for renovation.",
        "✅ All lines are running on time."
    ])
    
    return {"alerts": [alert] if alert else [], "line_statuses": statuses, "last_updated": datetime.now().strftime("%I:%M %p")}
...
def find_route(query: RouteQuery):
    ...
    # Use dmrc_complete for routing if available
    if dmrc_complete:
        route_result = dmrc_complete.find_route(from_st, to_st)
        if "error" in route_result:
            raise HTTPException(status_code=404, detail=route_result["error"])
        stations = route_result['path']
        num_stations = route_result['total_stations']
        steps = route_result['steps']
    else:
        # Fallback simulation if dmrc_complete is missing
        stations = [from_st, "Station A", "Station B", to_st]
        num_stations = 4
        steps = [
            {"line": "Yellow Line", "start": from_st, "end": "Station A"},
            {"line": "Blue Line", "start": "Station A", "end": to_st}
        ]

    # Calculate fare (using chatbot engine logic for full breakdown)
    fare = chatbot.calculate_fare(num_stations)
    
    # Build interchange list from dmrc_complete steps
    interchanges = []
    # steps[0] is first segment, steps[1] is second, etc.
    # Interchange happens at the end of a segment if there is a next segment
    for i in range(len(steps) - 1):
        current_step = steps[i]
        next_step = steps[i+1]
        interchanges.append({
            "station": current_step['end'],
            "from_line": current_step['line'],
            "to_line": next_step['line'],
            "lines": [current_step['line'], next_step['line']],
            "instruction": f"At {current_step['end']}, change from {current_step['line']} to {next_step['line']}"
        })
    ...
    response = {
        ...
        "segments": steps, # dmrc_complete steps are segments
...
def fuzzy_search_endpoint(q: str, limit: int = 8, threshold: int = 60):
    ...
    # Use dmrc_complete graph keys
    station_names = list(dmrc_complete.METRO_GRAPH.keys()) if dmrc_complete else list(STATION_FACILITIES.keys())
    
    # Use fuzzy search if available, else basic filter
    if FUZZY_SEARCH_AVAILABLE:
        results = fuzzy_search_station(q, station_names, limit=limit, threshold=threshold)
    else:
        # Basic fallback
        results = [{"name": s, "score": 100} for s in station_names if q.lower() in s.lower()][:limit]
...
def autocomplete_endpoint(q: str, limit: int = 5):
    ...
    if FUZZY_SEARCH_AVAILABLE:
        ...
        suggestions = [s for s in station_names if q.lower() in s.lower()][:limit]
...
def get_station(station_name: str):
    ...
    # (Simplified detail logic for now)
...
def get_all_stations_list():
    ...
    if dmrc_complete:
        all_stations = list(dmrc_complete.METRO_GRAPH.keys())
    else:
        # Fallback list
        all_stations = ["Rajiv Chowk", "Kashmere Gate", "New Delhi", "Chandni Chowk", "Hauz Khas"]
